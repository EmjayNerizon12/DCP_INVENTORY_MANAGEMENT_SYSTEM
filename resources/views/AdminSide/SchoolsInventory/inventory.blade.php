...
@section('content')
	...
	@php
		...
		$total_disposed_condition = 0;

		foreach ($school_items_count as $batch) {
		    foreach ($batch['items'] as $item) {
		        $batch_item_count++;
		        $condition_text = $item->dcpItemCurrentCondition->dcpCurrentCondition->name ?? '';
	...
	<div class="md:my-5 mx-0 my-0">
		<div class="mb-4 rounded-md border border-gray-200 bg-white p-4 shadow-md">
			<div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
				<div class="flex justify-start gap-2 items-center">
					<div class="h-10 w-10 bg-white p-3 shadow-lg rounded-full flex items-center justify-center">
					...
					<div>
						<h1 class="text-2xl font-bold text-gray-800 uppercase">DCP Inventory</h1>
						<div class="text-lg text-gray-600">
							Select a school to filter items.
						</div>
					</div>
				</div>

				<form method="GET" action="{{ route('index.SchoolsInventory') }}"
					class="grid w-full grid-cols-1 gap-4 md:grid-cols-2 xl:w-auto xl:min-w-[34rem] xl:justify-end">

					<div class="w-full">
						<div class="text-gray-700 font-normal text-md">
							School</div>
						<div class=" rounded-md">
							<select name="school" onchange="this.form.submit()" class="select2 form-input max-w-xs py-1">
								<option value=""> All </option>
								@foreach ($schools as $school)
									<option value="{{ $school->pk_school_id }}" {{ $selectedSchool == $school->pk_school_id ? 'selected' : '' }}>
					...
					<div class="w-full">
						<div class="text-gray-700 font-normal text-md">
							Budget Year</div>
						<select name="budget_year" onchange="this.form.submit()" class="form-input max-w-xs">
		...
		<div class="grid grid-cols-1 gap-3 mb-4 sm:grid-cols-2 xl:grid-cols-4">
			<div
				class="bg-white shadow-md rounded-md border border-gray-300 p-5 flex flex-col justify-between transition-all duration-200">
				<div class="flex items-center gap-4">
					<div class="dashboard-icon-container">
						<div class="dashboard-icon bg-blue-600">
							<svg viewBox="0 0 24 24" class="w-9 h-9" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path
								...
								<path d="M12 21V12" stroke="white" stroke-width="2"></path>
								<path d="M12 12L4 7.5" stroke="white" stroke-width="2"></path>
								<path d="M20 7.5L12 12" stroke="white" stroke-width="2"></path>
							</svg>
						</div>
					</div>
				</div>
				<div class="w-full mt-4">
					<p class="dashboard-card-label">Total Batches</p>
					<h3 class="dashboard-card-value">{{ $count }}</h3>
				</div>
			</div>
			...
			<div
				...
				<div class="flex items-center gap-4">
					<div class="dashboard-icon-container">
						<div class="dashboard-icon bg-green-600">
							<svg fill="none" viewBox="0 0 24 24" class="w-9 h-9" xmlns="http://www.w3.org/2000/svg">
								<path d="M4 7H21V10H4V7Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
								<path d="M5 10H19V18C19 19 18 20 17 20H7C6 20 5 19 5 18V10Z" stroke="white" stroke-width="2"
				...
				<div class="w-full mt-4">
					<p class="dashboard-card-label">Total Items</p>
					<h3 class="dashboard-card-value">{{ $batch_item_count }}</h3>
				</div>
			</div>
			...
			<div
				...
				<div class="flex items-center gap-4">
					<div class="dashboard-icon-container">
						<div class="dashboard-icon" style="background-color:#00A651;">
							<svg viewBox="0 0 20 20" class="w-9 h-9 text-white" xmlns="http://www.w3.org/2000/svg" fill="none">
								<path d="M6 10.5L8.5 13L14 7.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"
				...
				<div class="w-full mt-4">
					<p class="dashboard-card-label">Good Condition</p>
					<h3 class="dashboard-card-value">{{ $total_good_condition }}</h3>
				</div>
			</div>
			...
			<div
				...
				<div class="flex items-center gap-4">
					<div class="dashboard-icon-container">
						<div class="dashboard-icon" style="background-color:#7C3AED;">
							<svg viewBox="0 0 24 24" class="w-9 h-9" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M7 7H17V11C17 14.3137 14.3137 17 11 17H7V7Z" stroke="white" stroke-width="2" stroke-linecap="round"
									stroke-linejoin="round"></path>
								<path d="M10 17V20" stroke="white" stroke-width="2" stroke-linecap="round"></path>
								<path d="M7 20H13" stroke="white" stroke-width="2" stroke-linecap="round"></path>
							</svg>
						</div>
					</div>
				</div>
				<div class="w-full mt-4">
					<p class="dashboard-card-label">Total Disposed</p>
					<h3 class="dashboard-card-value">{{ $total_disposed_condition }}</h3>
				</div>
			</div>
		</div>
			...
			@if ($school_items->isNotEmpty())
				@foreach ($school_items as $batch)
					<div class="bg-white border border-gray-300 shadow rounded p-4 mb-4" x-data="{ open: false }">
						...
						@endphp
						<!-- Totals -->
						<div class="bg-white p-4 rounded-lg shadow-md border border-gray-300 md:inline-block hidden">
							<div class="flex flex-col md:flex-row flex-wrap gap-3">

								<!-- GOOD -->
								<div class="flex items-center gap-3">
									<div style="background: #00A651" class="rounded-md bg-white p-2 shadow">
										<svg viewBox="0 0 20 20" class="text-white h-8 w-8" xmlns="http://www.w3.org/2000/svg" fill="none">
								...
								<!-- DAMAGED -->
								<div class="flex items-center gap-3">
									<div style="background-color:#D62828" class="rounded-md bg-white p-2 shadow ">
										<svg viewBox="0 0 24 24" class="w-8 h-8 text-white" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
											<g id="SVGRepo_bgCarrier" stroke-width="0"></g>
											<g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
											<g id="SVGRepo_iconCarrier">
												<g>
								...
								<!-- TOTAL ITEMS -->
								<div class="flex items-center gap-3">
									<div class="rounded-md bg-blue-600 p-2 shadow">
								...
								<div class="flex items-center gap-3">
									<div style="background-color:#FFB400" class="rounded-md   p-2 shadow">
										<svg version="1.1" id="svg2" class="w-8 h-8 text-white" xmlns:dc="http://purl.org/dc/elements/1.1/"
											...
											xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1200 1200" enable-background="new 0 0 1200 1200"
								...
								<!-- FOR DISPOSAL -->
								<div class="flex items-center gap-3">
									<div class="rounded-md bg-indigo-600 p-2 shadow ">
										<svg fill="currentColor" class="w-8 h-8 text-white" viewBox="-6.7 0 122.88 122.88" version="1.1"
								...
								<!-- MISSING -->
								<div class="flex items-center gap-3">
									<div class="rounded-md bg-gray-500 p-2 shadow ">
										<svg class="w-8 h-8 text-white" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg"
								...
								<!-- NO STATUS -->
								<div class="flex items-center gap-3">
									<div class="rounded-md bg-white p-2 shadow border border-gray-300">
										<svg class="w-8 h-8 text-black" viewBox="0 0 50.8 50.8" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" fill="currentColor"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g style="stroke-width:1.00012;stroke-dasharray:none"> <path d="M12.965 29.31h10.92l-1.55 15.013c-.052.784.348.955.941 0l14.55-22.834" style="fill:none;stroke:#000000;stroke-width:3.175;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:none"></path> <path d="M37.826 21.49h-10.92l1.549-15.013c.052-.784-.348-.955-.941 0l-14.55 22.834" style="fill:none;stroke:currentColor;stroke-width:3.175;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:none"></path> </g> </g></svg>
									</div>
									<div>
						...
						<div x-show="open" x-transition class="mt-3 overflow-x-auto">
							<div class="grid gap-3">
								@foreach ($batch->dcpBatchItems ?? [] as $index => $item)
									@php
										$condition_id = \App\Models\DCPItemCondition::where('dcp_batch_item_id', $item->pk_dcp_batch_items_id)->value(
										    'current_condition_id',
										);

										$condition_name = $condition_id
										    ? \App\Models\DCPCurrentCondition::where('pk_dcp_current_conditions_id', $condition_id)->value('name')
										    : null;
										...
										$itemName = \App\Models\DCPItemTypes::where('pk_dcp_item_types_id', $item->item_type_id)->value('name');
									@endphp
									...
									<div
										...
										<!-- Code -->
										<div class="text-md md:col-span-2 font-medium">
											<span
