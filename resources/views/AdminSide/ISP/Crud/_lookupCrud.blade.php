<div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
	...
	<form action="{{ route('admin.isp.store') }}" method="POST" class="flex gap-2 mb-4">
		@csrf
					...
					<tr class="border-b align-top">
						<td class="px-2 py-2">
							<div class="flex flex-row gap-1 w-full items-center">
								<form action="{{ route('admin.isp.update') }}" method="POST" class="flex gap-2 flex-1">
									@csrf
									@method('PUT')
									<input type="hidden" name="type" value="{{ $type }}">
								...
								<form action="{{ route('admin.isp.delete', ['id' => $item->id, 'type' => $type]) }}" method="POST"
